...
vrp_folder_path = r"C:\Users\jerem\Filr\Meine Dateien\Projekte\LSME\Data\VRP XML"
excel_file = os.path.join(vrp_folder_path, "LSME_VRP.xlsx")
...
def process_xml_files_in_folder (vrp_folder_path, excel_file):
    ...
    # Entfernt die Leerzeichen und wandelt alle Werte in float um
    for row in range(1, vrp_sheet.max_row + 1):
    ...
    for row in range(1, vrp_sheet.max_row + 1):
        for col in range(2, vrp_sheet.max_column + 1):
            ...
            if isinstance(cell_value, str):
                try:
                    vrp_sheet.cell(row=row, column=col).value = float(cell_value.replace(" ", ""))
                except ValueError:
                    pass
...
#Gesangs-Tonumfang berechnen und in einer neuen Zeile hinzufügen
    ...
    vrp_sheet.cell(row=norm_profile_coverage_row +1, column=1).value = "Gesangs-Tonumfang"
...
#Gesangs-Dynamik berechnen und in einer neuen Zeile hinzufügen
    ...
    vrp_sheet.cell(row=norm_profile_coverage_row + 2, column=1).value = "Gesangs-Dynamikumfang"

    speaking_slope_row = None
    speech_pitch_max_row = None
    speech_pitch_min_row = None
    speech_dynamic_max_row = None
    speech_dynamic_min_row = None

    for row in range(1, vrp_sheet.max_row + 1):
        cell_value = vrp_sheet.cell(row=row, column=1).value
        if cell_value == "speaking_profile_slope":
            speaking_slope_row = row
        elif cell_value == "speaking_shout_pitch":
            speech_pitch_max_row = row
        elif cell_value == "speaking_soft_pitch":
            speech_pitch_min_row = row
        elif cell_value == "speaking_shout_volume":
            speech_dynamic_max_row = row
        elif cell_value == "speaking_soft_volume":
            speech_dynamic_min_row = row

    if speaking_slope_row:
        insert_row = speaking_slope_row + 1
        vrp_sheet.insert_rows(insert_row)
        vrp_sheet.insert_rows(insert_row + 1)

        num_columns = vrp_sheet.max_column

        # Sprech-Tonumfang berechnen
        for col in range(2, num_columns + 1):
            pitch_high = vrp_sheet.cell(row=speech_pitch_max_row, column=col).value
            pitch_low = vrp_sheet.cell(row=speech_pitch_min_row, column=col).value
            if isinstance(pitch_low, (int, float)) and isinstance(pitch_high, (int, float)):
                semitones = calculate_semitones(pitch_low, pitch_high)
            else:
                semitones = None
            vrp_sheet.cell(row=insert_row, column=col).value = semitones
        vrp_sheet.cell(row=insert_row, column=1).value = "Sprech-Tonumfang"

        # Sprech-Dynamikumfang berechnen
        for col in range(2, num_columns + 1):
            dynamic_high = vrp_sheet.cell(row=speech_dynamic_max_row, column=col).value
            dynamic_low = vrp_sheet.cell(row=speech_dynamic_min_row, column=col).value
            if isinstance(dynamic_low, (int, float)) and isinstance(dynamic_high, (int, float)):
                dynamic_range = calculate_dynamic(dynamic_low, dynamic_high)
            else:
                dynamic_range = None
            vrp_sheet.cell(row=insert_row + 1, column=col).value = dynamic_range
        vrp_sheet.cell(row=insert_row + 1, column=1).value = "Sprech-Dynamikumfang"
